#include "cmd_table_0x10.hpp"
#include "cmd_table_0x1b.hpp"
#include "cmd_table_0x1c.hpp"
#include "cmd_table_0x1d.hpp"
#include "cmd_table_0x1e.hpp"
#include "cmd_table_0x1f.hpp"

// 使用__attribute__((weak))定义入口函数表，这样可以不占内存
// 使用的时候直接定义一个强定义就可以了

#undef  HANDLER
#define HANDLER(code)	__attribute__((weak)) int cmd_handler_##code(uint8_t *Cmd,int CmdLen,int *pCmdLen,void (*tx)(char *buf,int len));
#undef  pHANDLER
#define pHANDLER(code)	cmd_handler_##code
...
int (* const gCmdHandler_Array[256])(uint8_t *Cmd,int CmdLen,int *pCmdLen,void (*tx)(char *buf,int len))=
{
